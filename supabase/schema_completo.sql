-- =====================================================================
-- Raiz | Schema COMPLETO e seguro de rodar mais de uma vez.
-- Pode colar tudo no SQL Editor do Supabase e rodar sem medo:
-- nao duplica nada e nao da erro se alguma parte ja existir.
-- =====================================================================
...
drop policy if exists "ler proprios checkins" on public.checkins;
create policy "ler proprios checkins" on public.checkins
  for select using (auth.uid() = user_id);

drop policy if exists "inserir proprios checkins" on public.checkins;
create policy "inserir proprios checkins" on public.checkins
  for insert with check (auth.uid() = user_id);

drop policy if exists "atualizar proprios checkins" on public.checkins;
create policy "atualizar proprios checkins" on public.checkins
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "apagar proprios checkins" on public.checkins;
create policy "apagar proprios checkins" on public.checkins
...
drop policy if exists "tarefas_casa proprias" on public.tarefas_casa;
...
drop policy if exists "compras_casa proprias" on public.compras_casa;
...
drop policy if exists "meta_passos proprios" on public.meta_passos;
